...
Reqvire implementation coverage reports show which **requirements** are implemented and which are still missing implementation evidence. In the semantic engineering graph, implementation evidence stays attached to implementable obligations rather than to high-level capabilities.
...
This keeps capability intent, stakeholder context, ontology context, and implementation-independent system ability separate from direct implementation tracking. Capability implementation coverage is reported by rolling up the requirements that specify each capability.
