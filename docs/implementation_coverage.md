...
Reqvire implementation coverage reports show which **requirements** are implemented and which are still missing implementation evidence.
...
This keeps product scope, stakeholder context, and ontology context separate from direct implementation tracking. Feature implementation coverage is reported by rolling up the requirements that specify each feature.
